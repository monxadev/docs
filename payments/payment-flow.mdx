...
### Option 1: Charge‑only (Quick Start)
...
- **Statuses**: `pending`, `requires_action`, `succeeded`, `failed`, `expired`, `canceled`
- **Actions enum**
...
### Option 2: Order + Charge (Quick Start)
...
| Event                    | Description                           |
| :----------------------- | :------------------------------------ |
| `charge.action_required` | Charge initiated, awaiting completion |
...
```json
{
  ...
  "data": {
    "object": {
      ...
      "client_reference_id": "INV-240001",
      "captured_at": "2025-10-07T09:11:22Z"
    }
  },
  "created": "2025-10-07T09:11:22Z",
  "signature": "t=1696660282,v1=af3c..."
}
```
...
```json
{
  ...
  "signature": "t=1696660325,v1=93df..."
}
```
...
| `amount`              | integer | Minor units                                                                          |
| `currency`            | string  | ISO 4217                                                                             |
| `status`              | string  | `pending` \| `requires_action` \| `succeeded` \| `failed` \| `expired` \| `canceled` |
| `payment_method_type` | string  | `qris` \| `bank_transfer` \| `ewallet` \| `card` (future)                            |
| `payment_method`      | object  | Method‑specific params (if provided)                                                 |
| `client_reference_id` | string  | Merchant‑side reference/trace ID                                                     |
...
| `qr_string`    | string | For QR flows (e.g., QRIS)                              |
